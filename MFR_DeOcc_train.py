...
if __name__ == '__main__':
    ...
    run_dir = './log/' + datetime.date.today().__str__() + '_0'
    ...
    trainFolder = dataset('./train_re.txt')
    trainLoader = torch.utils.data.DataLoader(trainFolder, batch_size=batch_size, shuffle=True, pin_memory=False,
                                              num_workers=0, drop_last=True)
    testFolder = dataset('./test_re.txt')
    ...
    model = M(netParams={'Ts': 1, 'tSample': 40}, inChannels=33 + 11, norm='BN').to('cuda')
    ...
    # 指定要冻结的层的数量
    num_layers_to_freeze = 25

    # 冻结前10个层的参数
    freeze_first_n_layers(model, num_layers_to_freeze)

    print(model)
    # Define dummy inputs with the specified shapes
    input1 = torch.randn(1,2, 256, 256,40).to('cuda')
    input2 = torch.randn(1, 44, 256, 256).to('cuda')

    # Use thop to calculate FLOPs and parameters
    flops, params = profile(model, inputs=(input1, input2))

    # Format the FLOPs and parameter counts to be more readable
    flops, params = clever_format([flops, params], "%.3f")
    print(f"THOP - FLOPs: {flops}, Params: {params}")


    # 使用 ptflops 计算相同的指标
    def input_constructor(input_res):
        return {'events': torch.randn((1, 2, 256, 256, 40)).to('cuda'),
                'frames': torch.randn((1, 44, 256, 256)).to('cuda')}


    macs, params = get_model_complexity_info(model, (1, 2, 256, 256, 40,1, 44, 256, 256), input_constructor=input_constructor,
    ...
    if os.path.exists(os.path.join(run_dir, 'checkpoint.pth.tar')):
        ...
        model_info = torch.load(os.path.join(run_dir, 'checkpoint.pth.tar'), map_location='cpu')
    ...
    for epoch in range(cur_epoch, epochs):
        ...
        for i, (event_vox, img, gt_img, mask) in enumerate(trainLoader):
            ...
            if i % max(int(train_total_iter / 10), 1) == 0:
                # print(output.shape, gt_img.shape)
        ...
        with torch.no_grad():
            ...
            for i, (event_vox, img, gt_img, mask) in enumerate(testLoader):
                ...
                # print(mask.shape,img.shape) torch.Size([1, 11, 256, 256]) torch.Size([1, 33, 256, 256])
                output = model(event_vox, torch.cat([img, mask], dim=1))
            ...
            message = '============Epoch %d test done, loss:%f, PSNR:%f, SSIM:%f============' % (
            epoch, avg[3], psnr, ssim)
            showMessage(message, os.path.join(run_dir, 'log.txt'))
...
"""
    nohup python VEFNet_train.py > myout1.txt 2>&1 &
    ...
    cd /data/guoqi/Event_Enhanced_DeOcc-main/
    conda activate esai
    CUDA_VISIBLE_DEVICES=3,4,6,7 nohup python VEFNet_train.py > myout2.txt 2>&1 &


    cd /root/Event_Enhanced_DeOcc-main/
    conda activate guoqi
    CUDA_VISIBLE_DEVICES=1,2,3 nohup python VEFNet_train.py > myout1.txt 2>&1 &



    cuda available devices
    CUDA_VISIBLE_DEVICES=1,2,3,5,6,7 nohup python VEFNet_train.py > myout1.txt 2>&1 &
    CUDA_VISIBLE_DEVICES=1,2,3,5 nohup python VEFNet_train.py > myout1.txt 2>&1 &

    CUDA_VISIBLE_DEVICES=1 python VEFNet_train.py 

    CUDA_VISIBLE_DEVICES=7,4,3,2,1,0 nohup python VEFNet_train.py > myout2.txt 2>&1 &
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 VEFNet_train.py 

    CUDA_VISIBLE_DEVICES=3,4 python VEFNet_train.py 

    CUDA_VISIBLE_DEVICES=0 nohup python VEFNet_train.py > myout2.txt 2>&1 &
